...
gtf1="$(annotation_predictor_gtf "$TSEBRA_GTF1_SOURCE" "$sample_id")"
gtf2="$(annotation_predictor_gtf "$TSEBRA_GTF2_SOURCE" "$sample_id")"
hints1="$(annotation_predictor_hints "$TSEBRA_GTF1_SOURCE" "$sample_id")"
hints2="$(annotation_predictor_hints "$TSEBRA_GTF2_SOURCE" "$sample_id")"
run_root="$(annotation_tsebra_runs_dir "$sample_id")"
timestamp="$(date +'%Y%m%d_%H%M%S')"
run_dir="$run_root/tsebra_$timestamp"
current_link="$(annotation_tsebra_current_dir "$sample_id")"
